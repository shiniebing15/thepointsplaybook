...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const DOMAIN_DATA = [
    ...
    },
    {
      ...
      category: 'events',
      tasks: [
        { name: 'Creative Ideas', points: 30, type: 'high' },
  ...
  if (canvas) {
    ...
    function initMolecules() {
      ...
      const divisor = isMobile ? 45000 : 32000;
      const count = Math.max(12, Math.min(60, Math.floor((width * height) / divisor)));
      ...
      const hexCount = Math.max(2, Math.min(8, Math.floor(width / (isMobile ? 180 : 280))));
      for (let i = 0; i < hexCount; i++) {
        molecules.push({
          ...
          radius: Math.random() * (isMobile ? 12 : 16) + (isMobile ? 14 : 20),
  ...
  // Dynamic height recalculation for expanded accordions during orientation change / resizing
  function recalculateOpenDomainHeights() {
    domainCards.forEach(card => {
  ...
  window.addEventListener('resize', () => {
    clearTimeout(window._domainResizeTimer);
    window._domainResizeTimer = setTimeout(recalculateOpenDomainHeights, 100);
  }, { passive: true });

  window.addEventListener('orientationchange', () => {
    setTimeout(recalculateOpenDomainHeights, 200);
  }, { passive: true });
